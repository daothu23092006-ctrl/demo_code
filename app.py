...
[data-testid="stNumberInput"] label, [data-testid="stSelectbox"] label { font-size: 1rem !important; font-weight: 700 !important; color: #1a1a1a !important; } 
/* Label to đậm cho các section */ 
.field-label { font-size: 1rem; font-weight: 700; color: #1a1a1a; margin-bottom: 0.4rem; margin-top: 0.75rem; display: block; } 
...
if not st.session_state.profile_done: 
    ...
    with col2:
        ...
        activity = st.selectbox("Mức độ vận động", ["Ít vận động", "Vận động nhẹ", "Vận động vừa", "Vận động nhiều"])
...
""")
    ...
    st.markdown(f"""
    <div style="background:#fff;border:1.5px solid #e8e8e8;border-left:4px solid {badge_color};border-radius:16px;padding:1.1rem 1.25rem;margin-bottom:0.75rem"> <div style="font-size:0.7rem;color:#aaa;font-weight:700;letter-spacing:.08em;text-transform:uppercase">BMI · Chỉ số khối cơ thể</div> <div style="font-size:0.72rem;color:#bbb;margin-bottom:0.5rem">Đánh giá tình trạng cơ thể dựa trên chiều cao và cân nặng</div> <div style="font-size:2.4rem;font-weight:800;color:#1a1a1a;line-height:1">{bmi:.1f}</div> <div style="margin:0.5rem 0"> <span style="background:{badge_bg};color:{badge_color};padding:0.25rem 0.8rem;border-radius:50px;font-size:0.78rem;font-weight:700">{bmi_class}</span> </div> <div style="background:#eee;border-radius:50px;height:7px;margin-top:0.6rem"> <div style="width:{bmi_pct}%;height:100%;background:{badge_color};border-radius:50px"></div> </div> <div style="font-size:0.68rem;color:#bbb;margin-top:0.3rem">Phạm vi chuẩn (châu Á): 18.5 – 22.9</div> </div> """, unsafe_allow_html=True)
    st.markdown(f"""
    <div style="background:#fff;border:1.5px solid #e8e8e8;border-left:4px solid #3a5bd9;border-radius:16px;padding:1.1rem 1.25rem;margin-bottom:0.75rem"> <div style="font-size:0.7rem;color:#aaa;font-weight:700;letter-spacing:.08em;text-transform:uppercase">🔥 TDEE · Tổng năng lượng tiêu hao</div> <div style="font-size:0.72rem;color:#bbb;margin-bottom:0.5rem">Lượng calo cơ thể đốt cháy mỗi ngày theo mức vận động của bạn</div> <div style="font-size:2.4rem;font-weight:800;color:#1a1a1a;line-height:1">{tdee:,.0f} <span style="font-size:0.9rem;font-weight:400;color:#aaa">Calo/ngày</span></div> <div style="margin:0.5rem 0"> <span style="background:#eef2ff;color:#3a5bd9;padding:0.25rem 0.8rem;border-radius:50px;font-size:0.78rem;font-weight:600">Mức vận động: {p['activity']}</span> </div> <div style="border-top:1px solid #f0f0f0;margin-top:0.75rem;padding-top:0.75rem;display:flex;justify-content:space-between;align-items:center"> <span style="font-size:0.8rem;color:#555">Mục tiêu của bạn ({p['goal']})</span> <span style="font-size:1rem;font-weight:800;color:#FF6B6B">{tdee_adj_preview:,.0f} Calo</span> </div> <div style="font-size:0.72rem;color:#3a5bd9;margin-top:0.2rem">{delta_label}</div> </div> """, unsafe_allow_html=True)
    ...
    for col, icon, val, lbl in [
        ...
        with col:
            st.markdown(f"""
    <div style="background:#fafafa;border:1.5px solid #f0f0f0;border-radius:14px;padding:0.65rem 0.4rem;text-align:center"> <div style="font-size:1.2rem">{icon}</div> <div style="font-size:1rem;font-weight:800;color:#1a1a1a;margin-top:0.15rem">{val}</div> <div style="font-size:0.62rem;color:#aaa;margin-top:0.1rem">{lbl}</div> </div> """, unsafe_allow_html=True)
    ...
    if st.session_state.show_suggestions:
        ...
        for meal_id, dishes in suggestions.items():
            ...
            for dish in dishes:
                ...
                st.markdown(f"""
    <div style="background:#fff;border-radius:20px;overflow:hidden;margin-bottom:1rem;box-shadow:0 2px 12px rgba(0,0,0,0.07)"> {img_tag} <div style="padding:1rem"> <div style="font-size:1.05rem;font-weight:800;color:#1a1a1a;margin-bottom:0.2rem">{dish['dish_name']}</div> <div style="font-size:0.75rem;color:#aaa;margin-bottom:0.75rem">{dish['dish_type']}</div> <div style="display:flex;gap:1.2rem;align-items:center;flex-wrap:wrap"> <span style="font-size:0.82rem;color:#e74c3c;font-weight:700">🔥 {dish['calo']} Calo</span> <span style="font-size:0.82rem;color:#2980b9;font-weight:600">💪 {dish['protein']}g đạm</span> <span style="font-size:0.82rem;color:#d35400;font-weight:600">🧈 {dish['fat']}g béo</span> <span style="font-size:0.82rem;color:#27ae60;font-weight:600">🌾 {dish['fiber']}g xơ</span> </div> <div style="margin-top:0.6rem;font-size:0.72rem;color:#bbb"> <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:{sc};margin-right:4px"></span> Điểm phù hợp: {dish['score']:.2f} </div> </div> </div> """, unsafe_allow_html=True)        
            st.write("")
