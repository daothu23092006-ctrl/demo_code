...
.block-container { max-width: 460px !important; padding: 0 1rem 2rem !important; margin: 0 auto !important; }
...
.dish-card { background: #fff; border-radius: 16px; border: 1.5px solid #f0f0f0; overflow: hidden; margin-bottom: 0.75rem; }
...
st.caption("Tính toán dinh dưỡng & gợi ý thực đơn cá nhân")
st.divider()
...
""")
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.metric(label="BMI", value=f"{bmi:.1f}", delta=bmi_class)
        # Progress bar BMI (14–32 range)
        bmi_pct = max(0.0, min(1.0, (bmi - 14) / (32 - 14)))
        st.progress(bmi_pct)
        st.caption("Chuẩn châu Á: 18.5 – 22.9")
    with c2:
        delta_map = {
            "Giảm cân": f"−{tdee - tdee_adj_preview:.0f} kcal vs TDEE",
            "Tăng cân": f"+{tdee_adj_preview - tdee:.0f} kcal vs TDEE",
            "Duy trì":  "= Giữ nguyên TDEE",
        }
        st.metric(
            label="🔥 TDEE",
            value=f"{tdee:,.0f} kcal",
            delta=f"Mục tiêu: {tdee_adj_preview:,.0f} kcal",
        )
    ...
    s1.metric("📏 Chiều cao", f"{p['height']:.0f} cm")
    s2.metric("⚖️ Cân nặng",  f"{p['weight']:.0f} kg")
    s3.metric("🎂 Tuổi",      f"{p['age']}")
    s4.metric(f"{gender_icon} Giới tính", p["gender"])
    ...
    if st.button("🍽️ Gợi ý thực đơn hôm nay", use_container_width=True, type="primary"):
        ...
        for meal_id, dishes in suggestions.items():
            ...
            st.markdown(f"**{icon} Bữa {meal_id}** — {target['calo']} kcal · {target['protein']}g protein")
