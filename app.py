...
# Cấu hình ẩn hẳn sidebar ngay từ đầu để tránh lỗi giao diện kẹt nút
st.set_page_config(
...
section[data-testid="stSidebar"] { display: none !important; } /* Loại bỏ hoàn toàn thanh bên trái */
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.session_state.menu_done:
        ...
        with st.expander("📊 Xem lại Chỉ số sức khoẻ & Bộ lọc đã chọn", expanded=False):
            st.markdown(f"**Thông tin gốc:** {p['height']:.0f}cm · {p['weight']:.0f}kg · {p['age']} tuổi ({p['gender']})")
            ...
            # --- ĐÂY NÈ BỒ! THÊM HAI NÚT QUAY LẠI RÕ RÀNG Ở ĐÂY ---
            btn_col1, btn_col2 = st.columns(2)
            with btn_col1:
                if st.button("🔄 Thay đổi bộ lọc món ăn", use_container_width=True):
                    st.session_state.menu_done = False
                    st.rerun()
            with btn_col2:
                if st.button("✏️ Sửa lại Hồ sơ sức khỏe", use_container_width=True):
                    st.session_state.profile_done = False
                    st.session_state.menu_done = False
                    st.session_state.show_suggestions = False
                    st.session_state.user_choices = {}
...
""", unsafe_allow_html=True)
        ...
        # ĐƯA NÚT SỬA HỒ SƠ RA MÀN HÌNH CHÍNH (KHI CHƯA BẤM GỢI Ý MÓN) ĐỂ LUÔN SỬA ĐƯỢC
        st.write("")
